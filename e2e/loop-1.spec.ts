...
test.describe('Issue #1: [REQ-9441420] [REQ-需求收集工具] AI辅助的熟人需求收集与澄清工具（邀请码风控+Notion知识库集成）', () => {
  ...
  test('邀请码风控验证与用户信息收集完整流程', async ({ page, request }) => {
    ...
    const exhaustedCreateResp = await request.post('/api/invite/manage', { headers: ADMIN_HEADERS, data: { code: exhaustedCode } });
    expect(exhaustedCreateResp.ok(), `创建邀请码${exhaustedCode}失败: ${await exhaustedCreateResp.text()}`).toBeTruthy();
    for (let i = 0; i < 3; i++) {
    ...
    const validCreateResp = await request.post('/api/invite/manage', { headers: ADMIN_HEADERS, data: { code: validCode } });
    expect(validCreateResp.ok(), `创建邀请码${validCode}失败: ${await validCreateResp.text()}`).toBeTruthy();
